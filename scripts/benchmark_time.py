...
results = {}  # attn_type -> list of times
...
for attn_type in attn_types:
    ...
    for seq_len in seq_lengths:
        ...
        model = CustomTransformer(
            ...
            fuse_kernels=False,  # отключаем компиляцию для чистоты
...
plt.title('Сравнение времени инференса (без torch.compile)')
plt.legend()
plt.grid(True)
os.makedirs('plots', exist_ok=True)
plt.savefig('plots/time_vs_seqlen_compare.png', dpi=150)
plt.show()
...
# Сравнение ускорения относительно multihead
print("\nУскорение относительно MultiHead:")
print(f"{'seq_len':>8} ", end="")
