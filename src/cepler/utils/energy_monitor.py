...
class EnergyMonitor:
    ...
    def start(self):
        """Начать измерение."""
        pynvml.nvmlInit()  # повторно на случай, если было сброшено
        self._start_energy = pynvml.nvmlDeviceGetTotalEnergyConsumption(self.handle)
        self._start_time = time.time()
        self.total_energy_joules = 0.0

    def stop(self):
        """Остановить измерение и вернуть отчёт."""
        ...
        end_time = time.time()
        energy_joules = (end_energy - self._start_energy) / 1000.0  # NVML возвращает миллиджоули
    ...
    def _get_peak_power(self):
        # Простой способ: читаем текущую мощность (не пиковую, но для оценки)
        try:
